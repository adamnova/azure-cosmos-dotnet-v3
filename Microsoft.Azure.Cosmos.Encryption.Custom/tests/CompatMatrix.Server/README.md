...
- `Tests/` — **MSTest per-cell harness**: one `dotnet test` case per cell (`CompatMatrixCellTests` +
  `driver-shared/MatrixHarness`). 65 cases: 45 matrix cases (39 cells + 3 equivalence + 2 tamper +
  1 version guard) plus 20 follow-up contract tests. Optional local runs skip the 45 matrix cases
...
#   emulator up             -> Passed: 65 (45 matrix + 20 follow-up), Failed: 0
#   emulator down, optional -> Skipped: 45, Passed: 20 follow-up, Failed: 0
